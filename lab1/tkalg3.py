...
def window(master):
    def calculate():
        f = alg3.calculate(float(fields['a_entry'].get()), float(fields['b_entry'].get()))
        result.config(text=f"Result f: {f:.4f}")
    ...
    root.title("Algorythm 3")
    root.geometry("300x330")

    image = tk.PhotoImage(file='lab1/alg3.png')
    formula = tk.Label(root, image=image, bg="white")
    formula.image = image
    formula.pack(anchor=tk.W, padx=10)

    fields['a_label'] = tk.Label(root, text="n")
    ...
    result = tk.Label(root, text="Result f:", bg = "white")
